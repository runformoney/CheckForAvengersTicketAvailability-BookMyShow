...
........'''
        ...
        self.msg_left_url = 'http://site24.way2sms.com/sentSMS?Token=' \
            + self.jsid
...
........'''
        ...
        if len(msg) > 139 or len(mobile_no) != 10 \
            or not mobile_no.isdecimal():  # checks whether the given message is of length more than 139

            return False  # or the mobile_no is valid

        self.payload = {  # inorder to visualize how I came to these payload,
                          # must see the NETWORK section in Inspect Element
                          # while messagin someone from your browser
            'ssaction': 'ss',
            'Token': self.jsid,
            'mobile': mobile_no,
            'message': msg,
    ...
    def sendLater(  # Function for future SMS feature.
        self,
        mobile_no,
        msg,
        date,
        ...
        if len(msg) > 139 or len(mobile_no) != 10 \
            or not mobile_no.isdecimal():
            return False

        dateparts = date.split('/')  # These steps to check for valid date and time and formatting
        timeparts = time.split(':')
        if int(dateparts[0]) < 1 or int(dateparts[0]) > 32 \
            or int(dateparts[1]) > 12 or int(dateparts[1]) < 1 \
            or int(dateparts[2]) < 2017 or int(timeparts[0]) < 0 \
            or int(timeparts[0]) > 23 or int(timeparts[1]) > 59 \
            or int(timeparts[1]) < 0:
            return False
        date = dateparts[0].zfill(2) + '/' + dateparts[1].zfill(2) \
            + '/' + dateparts[2]
